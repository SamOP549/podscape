...
const Pod = ({ podcast }) => {
    ...
    const rows = podcast[0].episodes.map((element, index) => (
        <tr className='hover:bg-[rgb(255,255,255,0.5)] group' key={element.title}>
...
export async function getServerSideProps(context) {
    ...
    let podcast = await Podcast.find({ slug: context.query.slug })
    // Pass data to the page via props
    return { props: { podcast: JSON.parse(JSON.stringify(podcast)) } }
}
