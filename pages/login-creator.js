...
const Login = () => {
    ...
    useEffect(() => {
      ...
      const handleSubmit = async (e) => {
        ...
        let res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/login-creator`, {
          method: 'POST', // or 'PUT'
          headers: {
        ...
        setPassword('')
        if (response.success) {
          localStorage.setItem("mycreator", JSON.stringify({ token: response.token, email: response.email }))
